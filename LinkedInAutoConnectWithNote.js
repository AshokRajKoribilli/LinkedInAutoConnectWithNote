Linkedin = {
	...
	scrollBottom: function (data, config) {
		...
		console.debug(
			"DEBUG: scrolling to top in " + config.scrollDelay + " ms",
		);
		setTimeout(
	...
	inspect: function (data, config) {
		...
		console.debug(
			"DEBUG: total search results found on page are " + totalRows,
		);
		if (totalRows >= 0) {
	...
	compile: function (data, config) {
		...
		if (!data.pageButtons || data.pageButtons.length === 0) {
			...
			console.info(
				"INFO: " + data.pageButtonTotal + " connect buttons found",
			);
			...
			console.debug(
				"DEBUG: starting to send invites in " +
					config.actionDelay +
					" ms",
			);
			setTimeout(() => {
